...
public class Interval implements Comparable<Interval>, Operatable<Interval> {
    ...
    public Interval(Temporal start, Temporal end) {
        this((LocalDateTime) LocalDateTime.now().adjustInto(start), (LocalDateTime) LocalDateTime.now().adjustInto(end));
    }
    ...
    public Interval plus(Duration duration) {
        ...
        return new Interval(start.plus(duration), end.plus(duration));
    }
